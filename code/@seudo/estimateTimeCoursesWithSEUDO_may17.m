...
k2 = 2*sigma2*(sum(log(lambdas0(lambdas0>0))) - log(1./p-1));
...
one_blob    = one_blob(imax+(-cropRad:cropRad),jmax+(-cropRad:cropRad));   % ... And crop the blob
...
normFactors = [prof_norms,ones(1,nBlobs)];                                 % Calculate the norms of all the factors used in the generative model
...
blobFits = nan(nBlobs,size(rois,2));
for rr = 1:size(rois,2)
    blobFits(:,rr) = lsqnonneg(blobMat,rois(:,1));
end
