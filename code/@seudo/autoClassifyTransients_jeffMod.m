function varargout = autoClassifyTransients_jeffMod(se,structSpec,varargin)
...
for cc = 1:length(whichCells)
    ...
    if size(T.times,1) == 0                                                % If there are no transients in this time range, return appropriate struct
        ...
        metric_out.resFracFalse  = nan;                                    % secondary metric that counts the relative weight of false transient residuals  
    ...
    srcPrf  = se.profiles(T.window(1):T.window(2),...                      % get profile for this cell
                                         T.window(3):T.window(4), cellID);
    oSrcPrf = se.profiles(T.window(1):T.window(2),...                      % also get the profiles for other cells (but keep them separate)
                       T.window(3):T.window(4),[1:cellID-1,cellID+1:end]);
    
    
    switch 1
        case 1 % cut to the smallest bounding rectangle
            winSubY = any(srcPrf>0,2);
            winSubX = any(srcPrf>0,1);
            
        case 2 % use all pixels in the shape calculation
            winSubY = true(size(srcPrf,1),1);
            winSubX = true(size(srcPrf,2),1);
    end
    
    % keep only the pixels in the window
    srcPrf  = srcPrf(winSubY,winSubX);                                     % Restrict the source profile to the given window...
    tPrfs   = T.(params.profileFieldName)(winSubY,winSubX,:);              %    ... as well as the transient profiles...
    oSrcPrf = oSrcPrf(winSubY,winSubX,:);                                  %    ... as well as the other source profiles
    
    
    % blur transient profiles
    if params.tpBlur > 0
        tPrfsForRR = convn(tPrfs,fspecial('gauss',[1 1]*3*params.tpBlur,params.tpBlur),'same');
    else
        tPrfsForRR = tPrfs;
    end
    ...
    % reshape
    srcPrf  = srcPrf(:);
    tPrfs   = reshape(tPrfs,wX*wY,[]);
    tPrfsForRR   = reshape(tPrfsForRR,wX*wY,[]);
    oSrcPrf = reshape(oSrcPrf,wX*wY,[]);
    oSrcPrf = oSrcPrf(:,sum(oSrcPrf)~=0);
    ...
    metric_out.resFracFalse  = 0;                                                 % secondary metric that counts the relative weight of false transient residuals  
    metric_out.resFracTrue = 0;
    metric_out.resFracAll = 0;
    
    
    fit_vals         = [srcPrf,oSrcPrf]\tPrfsForRR;                             % fit a weighted sum of all source profiles to the transient profile
    res_minus_other  = tPrfsForRR - oSrcPrf*fit_vals(2:end,:);                  % Calculate residual after removing all profiles except this source's
    res_minus_all    = tPrfsForRR - [srcPrf,oSrcPrf]*fit_vals;                  % Calculate residual after removing all profiles (including this source's)
    res_minus_all    = reshape(res_minus_all,[wY,wX,size(evts,1)]);        % Reshape
    
    metric_out.resNorms  = reshape(sum(sum(res_minus_all.^2,1),2),[],1);   
    metric_out.resNoCell = reshape(sum(res_minus_other.^2,1),[],1);
    metric_out.resRatios = metric_out.resNorms./metric_out.resNoCell;       
...
%             res_den           = sum(nans2zeros(res_evt(:)).^2);
            res_den           = sum(nans2zeros(res_minus_other).^2,1);
            for kk = 1:size(evts,1)                                        % Get auto-correlation for each variable
                TMPRES          = res_minus_all(:,:,kk);                             % Temporarily store the residual (in case of requiring modification)
                ...
                if isempty(params.dof)
                    dof_val = ceil(numel(srcPrf>0));
                else
                    dof_val = params.dof;
                    if dof_val < 1
                        dof_val = ceil(dof_val*numel(srcPrf>0));
                    end
                end
                ...
                metric_out.relres(kk) = (norm(nans2zeros(TMPRES(:)))./norm(res_minus_other(:,kk))).^2;
...
%                         avgWeight = fit_vals(1,kk)/sum(fit_vals(1,:));     % average weight is based on transient size
                    ...
                    if params.resRatioMean
                        metric_out.resFracFalse = metric_out.resFracFalse + ...               
                                    sum(nans2zeros(TMPRES(:)).^2); % secondary metric that counts the relative weight of false transient residuals   
                    else
                        metric_out.resFracFalse = metric_out.resFracFalse + ...             
                           sum(nans2zeros(TMPRES(:)).^2)/(res_off + sum(res_minus_other(:,kk).^2)); % secondary metric that counts the relative weight of false transient residuals       
                    end
                end
            end
            if ~params.resRatioMean
                if sum(metric_out.lbq==1) > 0
                    metric_out.resFracFalse = metric_out.resFracFalse/(res_off + sum(metric_out.lbq==1)); 
                end
            else
                if sum(metric_out.lbq==1) > 0
                    metric_out.resFracFalse = metric_out.resFracFalse/sum(res_den(metric_out.lbq==1)); 
                end
            end
            ...
            metric_out.fullVal = sum(tc_amps.*vec(metric_out.resRatios))/sum(tc_amps);
        ...
        case 'corr'
            ...
            for tt = 1:size(T.times,1)          
                corrs(tt) = corr(srcPrf,tPrfs(:,tt),'rows','pairwise');    % compute correlation, ignoring nans
            end 
            ...
            res_den_all  = sum(nans2zeros(res_minus_other).^2,1);
            res_den_true = sum(res_den_all(newClassification==seudo.valTrue));
            res_den_false  = sum(res_den_all(newClassification==seudo.valFalse));
            for kk = 1:size(evts,1)                                        % loop over transients
                if ismember(newClassification(kk),[seudo.valFalse seudo.valTrue])
                    switch newClassification(kk)
                        case seudo.valFalse
                            rfracFieldName = 'resFracFalse';
                            rd=res_den_false;
                        case seudo.valTrue
                            rfracFieldName = 'resFracTrue';
                            rd=res_den_true;
                        otherwise
                            error('code bug')
                    end
                    TMPRES          = res_minus_all(:,:,kk);                         % Temporarily store the residual
                    if params.ignoreZeros,TMPRES(zeroPix) = nan;end        % set to nan any pixels that have zero profile weight
                    ...
                    % primary metric that counts the weighted fraction of transients that are false
                    if ismember(newClassification(kk),seudo.valFalse)
                        metric_out.cfrac = metric_out.cfrac + avgWeight;       
                    end
                    
                    % secondary metric that counts the relative weight of transient residuals
                    if params.resRatioMean
                        metric_out.(rfracFieldName) = metric_out.(rfracFieldName) + ...
                            sum(nans2zeros(TMPRES(:)).^2)/rd;              
                        metric_out.resFracAll = metric_out.resFracAll  + ...
                            sum(nans2zeros(TMPRES(:)).^2)/sum(res_den_all);
                    else
                        metric_out.(rfracFieldName) = metric_out.(rfracFieldName) + ...
                           sum(nans2zeros(TMPRES(:)).^2)/sum(res_minus_other(:,kk).^2); % secondary metric that counts the relative weight of false transient residuals       
                        metric_out.resFracAll = metric_out.resFracAll  + ...               
                                    sum(nans2zeros(TMPRES(:)).^2)/sum(res_minus_other(:,kk).^2); 
                   end
                   
                end                            % loop over transients
%                 if newClassification(kk) == seudo.valFalse
...
%                                     sum(nans2zeros(TMPRES(:)).^2)/res_den; % secondary metric that counts the relative weight of false transient residuals   
%                     else
%                         metric_out.rfrac = metric_out.rfrac + ...             
...
%                 end
            ...
            if ~params.resRatioMean
                if sum(newClassification==seudo.valFalse) > 0
                    metric_out.resFracFalse = metric_out.resFracFalse/sum(newClassification==seudo.valFalse); 
                end
                if sum(newClassification==seudo.valTrue) > 0
                    metric_out.resFracTrue = metric_out.resFracTrue/sum(newClassification==seudo.valTrue); 
                end
                metric_out.resFracAll = metric_out.resFracAll/numel(newClassification); 
            end
